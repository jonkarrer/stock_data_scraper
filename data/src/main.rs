...
use database::{
    DailyStockBarModelEntry, DailyStockBarRepository, MonthlyStockBarModelEntry,
    MonthlyStockBarRepository, SqliteDb,
};
    ...
    for (symbol, bars) in bars_map {
        ...
        for (index, bar) in bars.iter().enumerate() {
            ...
            let bollinger_bands = if index < 20 {
                ...
                let prices: Vec<f32> = bars[(index - 20)..index].iter().map(|bar| bar.c).collect();
                tindi::BollingerBands::new(&prices, 20).expect("Bollinger Bands failed")
            };
